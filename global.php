...
\ ♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛♛*/
	...
	$TplClass->SetParam( 'CLUBNAME', 'Habbis VIP');
	$TplClass->SetParam( 'FECHAF', '2019');
	$TplClass->SetParam( 'FOOTER', 'Todos los derechos reservados a sus respectivos autores.' );
	$TplClass->SetParam( 'FOOTER2', 'HabboIS CMS' );
	...
	//BAN
        $ip = $Functions->CheckBanned();
        $checkban = $db->query("SELECT * FROM bans WHERE data = '".$user['id']."' AND type != 'prison' LIMIT 1");
   	while($mant = $checkban->fetch_array()){
      $_SESSION['LOGIN_ERROR'] = "Tu usuario ha sido baneado.";
       header("LOCATION: ". PATH ."/banned");
   }
   //END BAN
   
    //IP BAN
        $ip = $Functions->getRealIP();
        $checkban = $db->query("SELECT * FROM bans WHERE data = '".$ip."' LIMIT 1");
   	while($mant = $checkban->fetch_array()){
...
<div id="h28">HK2</div>
	...
	if($user['rank'] < 3){
	$db->query("UPDATE players SET vip_time = '0', vip_mes = '0', vip = '0', rank = '1'  WHERE id = '".$user['id']."' AND (UNIX_TIMESTAMP()-players.vip_time)>2592000 AND vip = '1' AND rank_vip = '1' LIMIT 1");
   	$db->query("DELETE from player_subscriptions where (UNIX_TIMESTAMP()-player_subscriptions.vip_time)>2592000 AND user_id = '".$user['id']."'");

   }
   
   if($user['rank'] > 2){
	$db->query("DELETE from player_subscriptions where (UNIX_TIMESTAMP()-player_subscriptions.vip_time)>2592000 AND user_id = '".$user['id']."'");
	$db->query("UPDATE players SET vip_time = '0', vip_mes = '0', vip = '0'  WHERE id = '".$user['id']."' AND (UNIX_TIMESTAMP()-players.vip_time)>2592000 AND vip = '1' AND rank_vip = '1' LIMIT 1");
   }
